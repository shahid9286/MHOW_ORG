        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="card card-primary card-outline">
                    <div class="card-body">
                        <form action="{{ route('admin.event.template.store', ['slug' => $slug]) }}" method="POST">
                            @csrf
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="body" name="body" rows="6" class="form-control"
                                    placeholder="Use placeholders like @{{ name }}, @{{ amount }}, etc.">{{ old('body') }}</textarea>
                                    @error('start_date')
                                        <p class="text-danger">{{ $message }}</p>
                                    @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="send_timing" id="send_timing" class="form-control">
                                    <option value="after-registration"
                                        {{ old('send_timing') == 'after_registration' ? 'selected' : '' }}>After Registration</option>
                                    <option value="7-days-before"
                                        {{ old('send_timing') == '7_days_before' ? 'selected' : '' }}>7 Days Before</option>
                                    <option value="3-days-before"
                                        {{ old('send_timing') == '3_days_before' ? 'selected' : '' }}>3 Days Before</option>
                                    <option value="before-24-hours"
                                        {{ old('send_timing') == 'before_24_hours' ? 'selected' : '' }}>Before 24 Hours</option>
                                </select>
                                    @error('start_date')
                                        <p class="text-danger">{{ $message }}</p>
                                    @enderror
