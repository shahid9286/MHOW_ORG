            ...
            <div class="col-md-12">
                <div class="card card-primary card-outline">
                    <div class="card-header">
                        <h3 class="card-title mt-1">{{ __('Edit Donation Form') }}</h3>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('donation-forms.update', $donationForm->id) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" id="title" class="form-control" name="title"
                                        ...
                                        value="{{ old('title', $donationForm->title) }}" required>
                                    @error('title')
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" id="subtitle" class="form-control" name="sub_title"
                                        ...
                                        value="{{ old('sub_title', $donationForm->sub_title) }}">
                                    @error('sub_title')
                                        <p class="text-danger">{{ $message }}</p>
                                    @enderror
                                </div>
                                    ...
                                    <div class="form-group">
                                        <div class="col-sm-12">
                                            ...
                                            <img class="mw-400 mb-3 show-img img-demo mt-2"
                                                src="{{ $donationForm->image ? asset($donationForm->image) : asset('assets/admin/img/img-demo.jpg') }}" 
                                                alt="" width="50px">
                                            @error('image')
                                                <p class="text-danger">{{ $message }}</p>
                                            @enderror
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" id="purpose" class="form-control" name="purpose"
                                        ...
                                        value="{{ old('purpose', $donationForm->purpose) }}">
                                    @error('purpose')
                                        <p class="text-danger">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <!-- order_no -->
                                <div class="col-md-6 mb-3">
                                    <label for="order_no">{{ __('Order Number') }}</label>
                                    <input type="number" id="order_no" class="form-control" name="order_no"
                                        placeholder="Enter order number" 
                                        value="{{ old('order_no', $donationForm->order_no) }}">
                                    @error('order_no')
                                        <p class="text-danger">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="compaign_id" id="compaign_id">
                                            <option value="" disabled>{{ __('Select Compaign') }}</option>
                                            @foreach ($compaigns as $compaign)
                                                <option value="{{ $compaign->id }}" 
                                                    {{ (old('compaign_id', $donationForm->compaign_id) == $compaign->id) ? 'selected' : '' }}>
                                                    {{ $compaign->title }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-12">
                                            <select class="form-control" name="status" id="status">
                                                <option value="publish" {{ old('status', $donationForm->status) == 'publish' ? 'selected' : '' }}>
                                                    {{ __('Publish') }}
                                                </option>
                                                <option value="unpublish" {{ old('status', $donationForm->status) == 'unpublish' ? 'selected' : '' }}>
                                                    {{ __('Unpublish') }}
                                                </option>
                                            </select>
                                            @error('status')
                                ...
                                <div class="col-md-12 mb-3">
                                    ...
                                    <div id="features-container">
                                        @php
                                            $oldAmounts = old('amount', $donationForm->amounts->toArray());
                                        @endphp
                                        
                                        @foreach($oldAmounts as $index => $amount)
                                ...
                                <div class="col-md-12 mb-3">
                                    ...
                                    <textarea id="description" class="form-control summernote" name="description"
                                        ...
                                        rows="6">{{ old('description', $donationForm->description) }}</textarea>
                                    @error('description')
                                        <p class="text-danger">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="col-12">
                                    <button class="btn btn-sm btn-primary mt-2 float-right">Update</button>
                                </div>
                            </div>
                        </form>
                    </div>
...
@section('js')
    <script>
        $(document).ready(function () {
            let featureIndex = {{ count(old('amount', $donationForm->amounts)) }};
