...
@section('title', ucwords(str_replace('-', ' ', $slug)) . ' | Edit Outline')
@section('content')
            ...
            <div class="col-md-12">
                ...
                <div class="card card-primary card-outline">
                    <div class="card-header">
                        <h3 class="card-title mt-1">{{ __('Outline Edit') }}</h3>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('admin.course.outline.update', ['id' => $courseOutline->id]) }}"
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <div class="input-group input-group-sm">
                                        ...
                                        <input type="text" id="title" class="form-control" name="title"
                                               placeholder="Enter Title" value="{{ old('title', $courseOutline->title) }}" required>
                                    </div>
                                    @error('title')
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <div class="input-group input-group-sm">
                                        ...
                                        <input type="text" id="subtitle" class="form-control" name="subtitle"
                                               placeholder="Enter Subtitle" value="{{ old('subtitle', $courseOutline->subtitle) }}" required>
                                    </div>
                                    @error('subtitle')
                                    <p class="text-danger">{{ $message }}</p>
                                    @enderror
                                ...
                                <div class="col-md-12 mb-3">
                                    ...
                                    <textarea id="description" class="form-control summernote" name="description" rows="6"
                                              placeholder="Enter Description">{{ old('description', $courseOutline->description) }}</textarea>
                                    @error('description')
                                    <p class="text-danger">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-12 mb-3">
                                    ...
                                    <div id="units-container">
                                        @foreach($courseOutline->units as $uIndex => $unit)
                                            <div class="unit-group border rounded p-3 mb-3" data-unit-index="{{ $uIndex }}">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" name="units[{{ $uIndex }}][title]" class="form-control form-control-sm"
                                                           value="{{ old("units.$uIndex.title", $unit->title) }}"
                                                           placeholder="Unit Title" required>
                                                </div>

                                                <div class="topics-container">
                                                    @foreach($unit->topics as $tIndex => $topic)
                                                        <div class="d-flex align-items-center mb-2 topic-group">
                                                            <input type="hidden" name="units[{{ $uIndex }}][topics][{{ $tIndex }}][id]" value="{{ $topic->id }}">
                                                            <input type="text" name="units[{{ $uIndex }}][topics][{{ $tIndex }}][title]"
                                                                   class="form-control form-control-sm mr-2"
                                                                   value="{{ old("units.$uIndex.topics.$tIndex.title", $topic->title) }}"
                                                ...
                                                <button type="button" class="btn btn-outline-secondary btn-sm add-topic" data-unit-index="{{ $uIndex }}">
                                                    Add Topic
                                                </button>
                                            </div>
                                        @endforeach
...
<script>
    $(document).ready(function() {
        let unitCounter = {{ $courseOutline->units->count() }};
        ...
        function addTopic(unitIndex, topicIndex = null) {
            ...
            unit.find('.topics-container').append(`
                <div class="d-flex align-items-center mb-2 topic-group">
                    <input type="text" name="units[${unitIndex}][topics][${tIndex}][title]"
                           class="form-control form-control-sm mr-2" placeholder="Topic Title" required>
                    <button type="button" class="btn btn-danger btn-sm remove-topic">×</button>
                </div>
            `);
